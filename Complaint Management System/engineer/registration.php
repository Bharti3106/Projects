...
if(isset($_POST['submit']))
{
  ...
  $password=md5($pass);
	...
	$status=1;
	$query=mysqli_query($con,"insert into engineers(fullName,engineerEmail,password,contactNo,status) values('$fullname','$email','$password','$contactno','$status')");
	$msg="Registration successfull. Now You can login !";
	 $msg1= " Hello $fullname,\n\n Welcome to Complaint Management ,  \n\nYour login Password is : $pass  ";
                      email_send($email," Your Password for Author Login", $msg1);
...
<html lang="en">
  <head>
    ...
    <title>CMS | Engineer Registration</title>
		         ...
		         <input type="text" class="form-control" placeholder="Full Name" name="fullname" required="required" autofocus>
		            ...
		            <input type="email" class="form-control" placeholder="Email ID" id="email" onBlur="userAvailability()" name="email" required="required">
		             <span id="user-availability-status1" style="font-size:12px;"></span>
		               Your Password will be send to your email... check you inbox<br>
